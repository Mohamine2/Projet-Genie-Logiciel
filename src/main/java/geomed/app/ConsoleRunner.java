...
public class ConsoleRunner {
    ...
    private static void manageHospitalsMenu(Scanner sc) {
        if (!geomed.app.security.SecurityContext.hasAccess(UserRole.ADMIN)) {
    ...
    private static void manageIncidentsMenu(Scanner sc) {
        if (!geomed.app.security.SecurityContext.hasAccess(UserRole.ADMIN, UserRole.DOCTOR)) {
    ...
    private static void inspectionMenu(Scanner sc) {
        ...
        switch (choice) {
            ...
            case 1:
                ...
                if (h != null) {
                    HospitalStats stats = geomed.app.algo.AnalyticsEngine.computeHospitalStats(h, mapManager.getIncidents());
                    ...
                    double density = geomed.app.algo.AnalyticsEngine.computeIncidentDensity(cellArea, stats.getAssignedIncidentsCount());
            ...
            case 2:
                ...
                if (vi != null) {
                    ...
                    if (geomed.app.security.SecurityContext.hasAccess(UserRole.DOCTOR, UserRole.ADMIN)) {
                    ...
                    if (vi.getClosestHospital() != null) {
                        ...
                        if (geomed.app.security.SecurityContext.hasAccess(UserRole.DOCTOR, UserRole.ADMIN)) {
                            ...
                            if (sc.next().equalsIgnoreCase("y")) {
                                DispatchEngine engine = new geomed.app.algo.DispatchEngine();
            ...
            case 3:
                ...
                if (tIdx >= 0 && tIdx < triangles.size()) {
                    ...
                    int countA = geomed.app.algo.AnalyticsEngine.getIncidentCountForHospital(hA, mapManager.getIncidents());
                    int countB = geomed.app.algo.AnalyticsEngine.getIncidentCountForHospital(hB, mapManager.getIncidents());
                    int countC = geomed.app.algo.AnalyticsEngine.getIncidentCountForHospital(hC, mapManager.getIncidents());
                    int imbalance = geomed.app.algo.AnalyticsEngine.getTriangleLoadImbalance(t, mapManager.getIncidents());
            ...
            case 2: {
                ...
                try {
                    if (choice == 1) {
                        ...
                        List<Hospital> hospitals = geomed.app.io.CsvSiteImporter.importFromCsv(path, startId);
                        ...
                        int startCount = mapManager.getIncidents().size() + 1;
                        List<VictimIncident> incidents = geomed.app.io.CsvIncidentImporter.importFromCsv(path, startCount);
    ...
    private static void importOsmMenu(Scanner sc) {
        ...
        try {
            ...
            geomed.app.io.MapImporterOSM.importFromOSM(mapManager, filePath);
